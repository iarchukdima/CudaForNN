...
__global__ void matmul_a_bt_kernel(float *A, float *B, float *C, int m, int n, int k) {
    ...
    for (int t = 0; t < (n + TILE_SIZE - 1) / TILE_SIZE; ++t) {
        ...
        for (int i = 0; i < TILE_SIZE; ++i) {
            sum += As[threadIdx.y][i] * Bs[threadIdx.x][i];  // Note Bs[threadIdx.x][i] for B.T
        }
...
__global__ void matmul_at_b_kernel(float *A, float *B, float *C, int m, int n, int k) {
    ...
    int row = blockIdx.y * TILE_SIZE + threadIdx.y;  // Now row is in n dimension
    int col = blockIdx.x * TILE_SIZE + threadIdx.x;  // col is in k dimension
    ...
    for (int t = 0; t < (m + TILE_SIZE - 1) / TILE_SIZE; ++t) {
        ...
        if (a_row < m && row < n) {
            As[threadIdx.y][threadIdx.x] = A[a_row * n + row];  // Transposed access
        } else {
            As[threadIdx.y][threadIdx.x] = 0.0f;
        }
        ...
        // Compute partial product
        for (int i = 0; i < TILE_SIZE; ++i) {
            sum += As[i][threadIdx.y] * Bs[i][threadIdx.x];  // Note As[i][threadIdx.y] for A.T
        }
